...
export default function ChatInput({ onSend, loading, disabled }: ChatInputProps) {
  ...
  return (
    <div className="space-y-2">
        ...
        <button
          ...
          className={`flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium transition-all ${
            eli5
              ...
              : 'bg-white shadow-sm dark:shadow-none dark:bg-white/5 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:border-white/20 hover:text-slate-600 dark:text-slate-300'
          }`}
        >
      ...
      {/* Input Row */}
      <div className="flex items-end gap-2 rounded-2xl border border-slate-200 dark:border-white/10 bg-white shadow-sm dark:shadow-none dark:bg-white/5 p-2 focus-within:border-violet-500/50 transition-colors">
        <textarea
          id="chat-input"
          value={text}
