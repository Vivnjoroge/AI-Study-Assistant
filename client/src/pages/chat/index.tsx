...
export default function ChatPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      {/* ── Sidebar ─────────────────────────────────────────────────────────── */}
      <aside
        className={`flex flex-col border-r border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d0d1a] transition-all duration-300 ${
          sidebarOpen ? 'w-64' : 'w-0 overflow-hidden'
        }`}
      >
        <div className="flex items-center justify-between p-4">
          <span className="text-sm font-semibold text-slate-600 dark:text-slate-300">Conversations</span>
          <button
            id="new-chat-btn"
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
              selectorOpen || selectedDocIds.length > 0
                ...
                : 'bg-white shadow-sm dark:shadow-none dark:bg-white/5 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-white/10 hover:border-slate-300 dark:border-white/20'
            }`}
          >
        ...
        {selectorOpen && (
          <div className="border-b border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d0d1a]/80 px-4 py-3 animate-fade-in">
            <DocumentSelector
              documents={documents}
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 max-w-lg mt-2">
                {[
                  ...
                  <button
                    ...
                    className="rounded-xl border border-slate-200 dark:border-white/10 bg-white shadow-sm dark:shadow-none dark:bg-white/5 px-4 py-3 text-left text-xs text-slate-600 dark:text-slate-300 hover:border-violet-500/30 hover:bg-violet-500/5 transition-all"
                  >
                    {prompt}
                  </button>
                ))}
            ...
            <div className="mx-auto max-w-3xl space-y-6">
              ...
              {loading && (
                <div className="flex gap-3">
                  ...
                  <div className="rounded-2xl rounded-tl-sm border border-slate-200 dark:border-white/10 bg-white shadow-sm dark:shadow-none dark:bg-white/5 px-4 py-3">
        ...
        {/* Input */}
        <div className="border-t border-slate-200 dark:border-white/10 bg-white dark:bg-[#0d0d1a]/80 p-4">
